...
- **Status-tinted surfaces (`--color-surface-danger` `#fde7e6`, `--color-surface-success` `#e1f1e7`, `--color-surface-warning` `#fff3e9`, `--color-surface-info` `#e0f2ff`)** — not in ClickUp's captured system.
  **Product addition (Phase 39).** Each is the matching accent above at a 12% mix over canvas, so a toast can read as its status from across the room without the accent itself carrying body text at full saturation. Toast title and description render `ink` on all four — measured at ≥11.68:1, clearing the 4.5:1 body-text floor with room to spare — rather than the accent, which measured 1.95:1–3.67:1 on these tints when Phase 39 first shipped it as the text colour and was corrected in the same phase. The border stays the plain accent, since a boundary carries no contrast requirement. The icon stays the plain accent for danger (3.34:1 on `surface-danger`) and success (3.67:1 on `surface-success`), both already past the 3:1 floor for a graphical object.
- **Brand amber strong (`--color-brand-amber-strong`, `#cd5f02`)** and **accent blue strong (`--color-accent-blue-strong`, `#007bd9`)** — not in ClickUp's captured system.
  **Product addition (Phase 39).** Plain `brand-amber` (`#fd9a46`) measures 1.95:1 on `surface-warning` and plain `accent-blue` (`#0091ff`) measures 2.82:1 on `surface-info` — both short of even the 3:1 floor for a graphical object, so neither can carry the warning or info toast's icon. The `-strong` variants exist solely for that icon: `brand-amber-strong` reads 3.68:1 on `surface-warning`, `accent-blue-strong` reads 3.79:1 on `surface-info`. Neither replaces its plain counterpart elsewhere — `brand-amber` still carries `StatusBadge`'s "Needs review" text on a different, darker surface, and `accent-blue` still carries `EXTRACTING`'s badge text on `surface-soft` — so the `-strong` pair is scoped to these two toast icons alone.
