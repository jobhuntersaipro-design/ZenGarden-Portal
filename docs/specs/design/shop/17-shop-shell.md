...
- **Footer** (`mt-3xl border-t border-hairline bg-surface`): four columns — wordmark + one line; *Shop*: All products and the first three categories; *Your account*: Sign in, My orders, Request an account (→ `shopHref.signIn()` this phase; Phase 18 points it at `/checkout`); *Contact*: `ZEN_GARDEN_PHONE`, `ZEN_GARDEN_EMAIL`, `ZEN_GARDEN_ADDRESS` — each row omitted when its variable is unset. Bottom rule: "© 2026 Zen Garden. All prices in Malaysian Ringgit." Stacks to two columns below `md`, one below `sm`.
