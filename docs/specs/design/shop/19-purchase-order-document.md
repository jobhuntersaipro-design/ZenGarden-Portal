...
`PurchaseOrderPdf` is the artboard in `@react-pdf/renderer` primitives (`Document`, `Page size="A4"`, `View`, `Text`), 48pt page padding, the masthead with the wordmark in two `Text` runs (gradient text is not available in PDF; *Zen* prints in `#7612fa` — the brand purple as **text**, which the design system permits — and *Garden* in ink), **PURCHASE ORDER** and the number; the four-cell meta strip; Buyer / Supplier; the line grid `34 / 132 / 1fr / 64 / 96 / 104` pt; totals right-aligned in a 300pt column; *Notes from the buyer* (`—` when none); the two signature rules; the footer line "Generated by Zen Garden on {generatedAt}" and the reference. Colours are the seven hex values from `_parts.md` declared once in `render.tsx` as a `PDF_COLORS` map — the one file in the repository allowed raw hex, because a PDF has no stylesheet; say so in its header comment.
